...
//优化push.apply（_，NodeList）
	...
	）;
        ...
        / **
        *创建有限大小的键值缓存
        * @returns {function（string，object）}使用后将对象数据存储到自身后返回
		...
		函数（标签，上下文）{
            ...
            if（（support.qsa = rnative.test（document.querySelectorAll）））{
				...
				“<select disabled ='disabled'> <option /> </ select>”;
                    ...
                    // FF 3.5  - ：enabled /：禁用和隐藏元素（隐藏元素仍然启用）
                    // IE8在这里抛出错误，不会看到后面的测试
...
jQuery.extend（{
    ...
    // Not public  - 生成一个queueHooks对象，或者返回当前的一个
